...
class scNetwork(nn.Module):
    ...
    def __init__(self, input_dim, z_dim, encodeLayer=[256, 128], decodeLayer=[128, 256], 
        ...
        self.bce_loss = nn.BCELoss()  # BCE损失
    ...
    def spatial_loss(self, q_batch, edge_index, batch_start, batch_end):
        if edge_index is None or edge_index.shape[0] == 0:
            return torch.tensor(0.0, device=q_batch.device)

        mask = (
            (edge_index[:, 0] >= batch_start)
            & (edge_index[:, 0] < batch_end)
            & (edge_index[:, 1] >= batch_start)
            & (edge_index[:, 1] < batch_end)
        )
        local_edges = edge_index[mask]
        if local_edges.shape[0] == 0:
            return torch.tensor(0.0, device=q_batch.device)

        src = local_edges[:, 0] - batch_start
        dst = local_edges[:, 1] - batch_start
        return F.mse_loss(q_batch[src], q_batch[dst])
    ...
    def fit(self, X, n_clusters, generate, margin, ratio,
            ...
            spatial_pairs=None, spatial_weight=0.0):
        
        self.train()
        
        X = torch.tensor(X, dtype=torch.float)
        if spatial_pairs is not None and len(spatial_pairs) > 0:
            spatial_pairs = torch.as_tensor(spatial_pairs, dtype=torch.long, device=self.device)
        else:
            spatial_pairs = None
        ...
        for epoch in range(num_epochs):
            ...
            bce_loss_val = cluster_loss_val = spatial_loss_val = 0.0
            
            for batch_idx in range(num_batch):
                batch_start = batch_idx * batch_size
                batch_end = min((batch_idx + 1) * batch_size, num)
                xbatch = X[batch_start:batch_end]
                pbatch = p[batch_start:batch_end]
                ...
                if spatial_pairs is not None and spatial_weight > 0:
                    spatial_loss = self.spatial_loss(qbatch, spatial_pairs, batch_start, batch_end)
                else:
                    spatial_loss = torch.tensor(0.0, device=self.device)

                loss1 = cluster_loss * self.gamma1 + bce_loss + spatial_weight * spatial_loss
                loss1.backward()
                optimizer.step()

                cluster_loss_val += cluster_loss.item() * len(inputs)
                bce_loss_val += bce_loss.item() * len(inputs)
                spatial_loss_val += spatial_loss.item() * len(inputs)
            ...
            if spatial_pairs is not None and spatial_weight > 0:
                print("\t\tSpatial Loss: %.5f" % (spatial_loss_val / num))
            if tri_num_batch > 0:
                print("\t\tTriplet Loss: %.5f" % (triplet_loss_val / num))

            total_loss = (
                bce_loss_val
                + cluster_loss_val * self.gamma1
                + triplet_loss_val * self.gamma2
                + spatial_loss_val * spatial_weight
            )
